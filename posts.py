...
class Posts:
    def add_post(self, post_id, message_id, channel_id):
        ...
        c.execute(
            'INSERT INTO "posts" (message_id, post_id, channel_id) VALUES (?, ?, ?)',
            (
                message_id,
                post_id,
                channel_id,
    ...
    def add_report(self, s_id, message_id, channel_id):
        ...
        c.execute(
            'INSERT INTO "reports" (message_id, submission_id, channel_id) VALUES (?, ?, ?)',
            (message_id, s_id, channel_id),
        )
        conn.commit()
        c.close()
    ...
    def report_was_resolved(self, s_id):
        ...
        c.execute(
            "SELECT submission_id FROM reports WHERE submission_id=? AND resolved=1",
            (s_id,),
        )
        result = c.fetchone()
        if result is None:
    ...
    def unresolve_report(self, s_id, message_id, channel_id):
        ...
        c.execute(
            "UPDATE reports SET resolved=0, message_id=?, channel_id=? WHERE submission_id=?",
            (
                message_id,
                channel_id,
                s_id,
    ...
    async def mark_post_resolved(self, post_id, c=None):
        ...
        conn.commit()
        c.execute(
            "SELECT message_id, channel_id FROM posts WHERE post_id=?", (post_id,)
        )
        result = c.fetchone()
        if result is None:
            return True
    ...
    async def mark_report_resolved(self, s_id, c=None):
        ...
        conn.commit()
        c.execute(
            "SELECT message_id, channel_id FROM reports WHERE submission_id=?", (s_id,)
        )
        result = c.fetchone()
        if result is None:
            return True
